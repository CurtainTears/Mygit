...
public class SDgame extends JFrame implements ActionListener, KeyListener {
	...
	public static boolean judge(int[][] item) {
		for (int i = 0; i < 9; i++) {
	...
	public static boolean judgeLine(int[] item) {
		int[] temp = new int[9];
		for (int i = 0; i < 9; i++) {
			temp[i] = item[i];
		}
	...
	public static int[][] transformOfItems(int[][] oriItem) {
	...
	public void showNumOnButtons(int[][] item) {
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				if (item[i][j] == 0)
	...
	public static int[][] getCompleteItems() {
		int[][] temp = { 
				{ 7, 1, 6, 3, 5, 8, 4, 2, 9 },{ 8, 4, 9, 2, 6, 7, 3, 1, 5 }, { 3, 5, 2, 4, 1, 9, 6, 8, 7 },
